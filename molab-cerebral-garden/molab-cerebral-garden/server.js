/**
 * MoLab Cerebral Garden — unified server.
 *
 * Serves:
 *  - the app itself (public/index.html) — camera-gesture "garden" experience,
 *    gated behind a Rs. 110 / 3-minute-session payment
 *  - the admin panel (public/admin.html) for uploading a fresh payment QR
 *  - the payment-gate API (create session / poll status / bank webhook)
 *
 * Deploy this whole folder as a single Render Web Service (or any Node host):
 *   Build command: npm install
 *   Start command: node server.js
 *
 * Env vars to set before going live:
 *   ADMIN_PASSWORD   - password for the /admin.html panel (required, change from default!)
 *   WEBHOOK_SECRET   - shared secret your bank-notification listener sends in x-webhook-secret
 *
 * IMPORTANT — most free/standard hosts have an EPHEMERAL filesystem: a new
 * deploy or a restart wipes anything written at runtime, including an
 * uploaded QR image. That means whenever you push a new commit, or the
 * service restarts after being idle, the QR reverts to whatever's committed
 * in public/qr-current.jpg and data/qr-meta.json. Re-upload the QR after any
 * redeploy, or add a persistent disk if your host supports one.
 *
 * IMPORTANT — no real payment QR is included. public/qr-current.jpg ships as
 * a clearly-labelled placeholder ("QR NOT SET"). Upload your own Raast /
 * JazzCash / Easypaisa receiving QR through /admin.html before taking real
 * payments — never reuse someone else's QR code, since that would route
 * customer money into an account you don't control.
 */
...
const SESSION_TTL_MS = 5 * 60 * 1000; // how long a payment ref stays open waiting for a match
const PRICE_PKR = 110;
const EXPECTED_AMOUNT_STRINGS = ['110.00', '110', 'PKR 110', 'Rs. 110', 'Rs 110'];
...
if (!fs.existsSync(ANALYTICS_PATH)) {
  fs.writeFileSync(ANALYTICS_PATH, JSON.stringify({
    sessionsByDate: {},   // "YYYY-MM-DD" -> count of 3-min sessions started that day
...
app.get('/api/price-info', (req, res) => {
  res.json({ pricePkr: PRICE_PKR, sessionMinutes: 3 });
});
...
// Generic bank-notification webhook. Point your Android SMS/notification
// listener (e.g. MacroDroid/Tasker) here for whichever bank/wallet you use.
app.post('/api/mcb-webhook', (req, res) => {
  ...
  if (!looksLikeCredit || !amountMatches){
    return res.status(400).send(`Notification did not match a ${PRICE_PKR} PKR credit`);
  }

  // Replay protection: bank notifications include a Tx ID unique to that
  // transfer. If we've already processed this exact transaction (e.g. the
  // listener firing twice on the same SMS), don't double-credit.
  ...
  // Primary match: sender name. Bank "received from <NAME> <BANK> ..." style
  // wording doesn't cleanly separate name from bank, so instead of parsing
...
// Called by the client the moment a 3-min session actually starts, so admin
// can see how many sessions ran each day.
...
/* ---------------- Fallback to the app for any unmatched route ---------------- */
app.get('*', (req, res) => {
  res.sendFile(path.join(PUBLIC_DIR, 'index.html'));
});

app.listen(PORT, () => {
  console.log(`MoLab Cerebral Garden running on port ${PORT}`);
  if (ADMIN_PASSWORD === 'changeme123') console.log('⚠️  ADMIN_PASSWORD is still the default — set it in your host\'s env vars.');
  if (WEBHOOK_SECRET === 'CHANGE_ME_TO_A_LONG_RANDOM_STRING') console.log('⚠️  WEBHOOK_SECRET is still the default — set it in your host\'s env vars.');
  console.log('⚠️  public/qr-current.jpg is a placeholder — upload your real payment QR via /admin.html before taking real payments.');
});
